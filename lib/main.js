(
	function(){
		...
		,	'	export NODE_ENV={{env}}'
		,	'   #PORT=80'
		,	'   $foreverApp start --sourceDir=$nodeAppDir --pidFile $pidFile -l $logFile -a -d -c "$command" $nodeApp'
		,	'   RETVAL=$?'
		,	'}'
		,	''
		,	'restart() {'
		,	'	echo -n "Restarting $nodeApp"'
		,	'	$foreverApp restart $nodeAppDir$nodeApp'
		,	'	RETVAL=$?'
		,	'}'
		,	''
		,	'stop() {'
		,	'	echo -n "Shutting down $nodeApp"'
		,	'   $foreverApp stop $nodeAppDir$nodeApp'
		,	'   RETVAL=$?'
		,	'}'
		,	''
		,	'status() {'
			...
			init: function(){
				...
				try{
					...
					if(package.main && package.name){
						...
						this.packageInfo.version = package.version;
				...
				this.options = {	app: path.join(process.cwd() + '/' + this.packageInfo.appjs),
									...
									logfile:'/var/log/' + this.packageInfo.name + '.log',
				...
				program
					...
					.option('-a, --app [path]', 'The node.js applications main file')
					...
					.option('-d, --dir [path]', 'The location of node.js applciations main file')
			},
			/**
